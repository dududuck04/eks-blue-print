...
module "user_data" {
  ...
  enable_user_data           = var.enable_user_data
  pre_bootstrap_user_data    = var.pre_bootstrap_user_data
  post_bootstrap_user_data   = var.post_bootstrap_user_data
  bootstrap_extra_args       = var.bootstrap_extra_args
  user_data_template_path    = var.user_data_template_path

  cloudinit_post_nodeadm_path  = var.cloudinit_post_nodeadm_path
  cloudinit_pre_nodeadm_path   = var.cloudinit_pre_nodeadm_path
}
...
resource "aws_launch_template" "this" {
  ...
  depends_on = [module.eks_cluster_node_group_role]
...
resource "aws_eks_node_group" "this" {
  ...
  # Required
  cluster_name  = var.cluster_name
  node_role_arn = var.create_iam_role ? module.eks_cluster_node_group_role.aws_iam_role_arn[0] : var.iam_role_arn
...
locals {
  ...
  node_role_name = coalesce(var.iam_role_name, "${var.name}-role")
...
module "eks_cluster_node_group_role" {
  source = "../../../iam"

  create                         = var.create && var.create_iam_role
  name                           = var.iam_role_use_name_prefix ? null : local.node_role_name
  name_prefix                    = var.iam_role_use_name_prefix ? "${local.node_role_name}-" : null
  ...
  managed_policy_arns = concat(
    ...
    values(local.ipv4_cni_policy),
    values(local.ipv6_cni_policy),
    try("${local.iam_role_policy_prefix}/${var.iam_role_additional_policies}", [])
  )

  # (선택) Custom Policy 처리를 위해 여전히 policy 맵을 넘겨줄 수 있습니다.
  policy = var.custom_iam_role_additional_policies
