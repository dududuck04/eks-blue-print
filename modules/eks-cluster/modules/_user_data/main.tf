...
locals {
  ...
  enable_cloudinit_user_data = var.enable_user_data && local.user_data_type == "al2023" ? true : false

  enable_bootstrap_user_data = var.enable_user_data && local.user_data_type == "linux" ? true : false
  ...
  user_data_type_to_rendered = {
    ...
    bottlerocket = {
      user_data = var.create && local.user_data_type == "bottlerocket" && (local.enable_bootstrap_user_data || var.user_data_template_path != "" || var.bootstrap_extra_args != "") ? local.user_data : ""
    }
    linux = {
      user_data = var.create ? try(data.cloudinit_config.linux_eks_managed_node_group[0].rendered, local.user_data) : ""
    }
    windows = {
      user_data = var.create && local.user_data_type == "windows" && (local.enable_bootstrap_user_data || var.user_data_template_path != "" || var.pre_bootstrap_user_data != "") ? local.user_data : ""
    }
  }
}
...
data "cloudinit_config" "linux_eks_managed_node_group" {
  count = var.create && local.user_data_type == "linux" && var.is_eks_managed_node_group && local.enable_bootstrap_user_data && var.pre_bootstrap_user_data != "" && var.user_data_template_path == "" ? 1 : 0
...
locals {
  pre_nodeadm = concat(
      var.cloudinit_pre_nodeadm_path  != null ? [
      {
        content      = file(var.cloudinit_pre_nodeadm_path)
        content_type = "text/x-shellscript"
      }
    ] : [],
  ...
  nodeadm = var.is_eks_managed_node_group ? [] : [
    {
  ...
  post_nodeadm = concat(
    [
      {
        content      = templatefile("${path.module}/../../templates/al2023_user_data_ec2_tagging.tpl", {
          name_prefix = var.node_name
        })
        content_type = "text/x-shellscript"
      }
    ],
      var.cloudinit_post_nodeadm_path != null ? [
      {
        content      = file(var.cloudinit_post_nodeadm_path)
        content_type = "text/x-shellscript"
      }
    ] : [],
  ...
  nodeadm_cloudinit = local.enable_cloudinit_user_data ? concat(
    local.pre_nodeadm,
    local.nodeadm,
    local.post_nodeadm
  ) : var.cloudinit_pre_nodeadm
