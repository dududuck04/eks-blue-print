...
resource "helm_release" "argocd_application" {
  ...
  # ArgoCD addons 의 valueFiles 로 사용할 파일명 입력
  # 미입력시 values.yaml, {revision}-values.yaml
  set {
...
# ---------------------------------------------------------------------------------------------------------------------
# Private Repo Access via GITHUB TOKEN
# ---------------------------------------------------------------------------------------------------------------------

resource "kubernetes_secret" "argocd_gitops_token" {
  for_each = { for k, v in var.applications : k => v if try(v.github_token_secret_name, null) != null }
  ...
  data = {
    insecure      = lookup(each.value, "insecure", false)
    username      = jsondecode(data.aws_secretsmanager_secret_version.github_token_version[each.key].secret_string)["username"]
    password      = jsondecode(data.aws_secretsmanager_secret_version.github_token_version[each.key].secret_string)["token"]
    type          = "git"
    url           = each.value.repo_url
  }
