...
resource "kubernetes_service_account_v1" "irsa" {
  ...
  metadata {
    ...
    namespace   = try(kubernetes_namespace_v1.irsa[0].metadata[0].name, var.kubernetes_namespace)
    annotations = var.irsa_iam_policies != null || var.irsa_iam_policies_json != null ? { "eks.amazonaws.com/role-arn" : aws_iam_role.irsa[0].arn } : null
  }
...
resource "aws_iam_role" "irsa" {
  count = var.irsa_iam_policies != null || var.irsa_iam_policies_json != null ? 1 : 0

  name        = try(coalesce(var.irsa_iam_role_name, format("%s-%s-%s", var.eks_cluster_id, trim(var.kubernetes_namespace, "-*"), trim(var.kubernetes_service_account, "-*"))), null)
...
# ---------------------------------------------------------------------------------------------------------------------
resource "aws_iam_policy" "irsa_policy_json" {
  count = var.irsa_iam_policies_json != null ? length(var.irsa_iam_policies_json) : 0
  description = "IAM Policy for Workloads"
  policy      = var.irsa_iam_policies_json[count.index]
}

resource "aws_iam_role_policy_attachment" "irsa_json" {
  count = var.irsa_iam_policies_json != null ? length(var.irsa_iam_policies_json) : 0

  policy_arn = aws_iam_policy.irsa_policy_json[count.index].arn
  role       = aws_iam_role.irsa[0].name
}
