...
variable "costc" {
  description = "비용 센터(Cost Center) 또는 비용 관련 태그"
  type        = string
  default     = ""
}

variable "tags" {
  description = "모든 리소스에 공통으로 추가할 태그 (키-값 맵)"
  type        = map(string)
  default     = {}
}

variable "create" {
  description = "모든 리소스 생성 여부 (모듈 전체 동작을 켜거나 끌 수 있음)"
  type        = bool
  default     = true
}
...
variable "github_revision" {
  description = "테라폼 코드 브랜치 (revision)"
  type        = string
  default     = ""
}

########################################
# 3. Helm 배포 관련 (Karpenter 차트 등)
########################################

variable "name" {
  description = "Helm Release 이름 (예: karpenter)"
  type        = string
  default     = "karpenter"
}

variable "repository" {
  description = "Helm 차트를 가져올 리포지토리 URL"
  type        = string
  default     = ""  # 예시
}
...
variable "aws_load_balancer_controller_image_path" {
  description = "Custom image repository path for AWS Load Balancer Controller (e.g. 123456789012.dkr.ecr.ap-northeast-2.amazonaws.com/aws-load-balancer-controller)"
  type        = string
  default     = ""
}

variable "aws_load_balancer_controller_image_tag" {
  description = "Image tag for AWS Load Balancer Controller (e.g. v2.11.0)"
  type        = string
  default     = ""
}

variable "aws_load_balancer_controller_helm_chart" {
  description = "Helm Chart Version for AWS Load Balancer Controller (e.g. v1.11.0)"
  type        = string
  default     = ""
}
...
variable "argocd_alb_security_group_name" {
  description = "ArgoCD ALB 보안 그룹 이름"
  type        = string
  default     = "argocd-alb-sg"  # 기본값 설정 가능
}

variable "argocd_alb_security_group_name_use_name_prefix" {
  type        = bool
  default     = false
}

variable "argocd_alb_security_group_description" {
  type        = string
  default     = "ArgoCD ALB Security Group"
}
...
variable "argocd_admin_secret" {
  type        = string
  description = "ArgoCD가 사용할 AWS Secrets Manager에 생성될 Secret name"
  default     = "argocd_admin_password"
}
...
variable "argocd_admin_secret_name" {
  description = "ArgoCD Admin 비밀번호를 저장할 AWS Secrets Manager의 Secret 이름"
  type        = string
  default     = "argocd_admin_password"
}

variable "create_argocd_admin_secret" {
  description = "ArgoCD Admin Secret을 생성할지 여부"
  type        = bool
  default     = true
}


variable "argocd_login_url" {
  description = "Argo CD 접근 URL"
  type        = string
  default     = ""
}

variable "argocd_ingress_alb_name" {
  description = "Argo CD Ingress ALB 이름"
  type        = string
  default     = ""
}
...
variable "addons_repo_url" {
  description = "ArgoCD application Git repository URL"
  type        = string
}

variable "addons_repo_path" {
  description = "Path inside the Git repository where the Helm/Kustomize config is located"
  type        = string
}

variable "addons_repo_revision" {
  description = "Branch or tag to use in the Git repository"
  type        = string
  default     = "main"
}

variable "addons_repo_token" {
  description = "GitHub token secret name in SecretsManager for accessing private repo"
  type        = string
}
...
variable "argocd_kms_key_id" {
  type        = string
  default     = null
  description = <<-EOT
    (Optional) The KMS Key ID or ARN to use for encrypting the ArgoCD admin secret in AWS Secrets Manager.
    If not specified, AWS will use the default AWS managed key: "aws/secretsmanager".
    You can provide a CMK ARN or an alias (e.g., "alias/argocd-secret-key").
  EOT
}
